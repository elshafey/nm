...
function set_locale() {
    ...
    $url = UrlsTable::getOneBy('routed', urldecode(implode('/', $CI->uri->segments)));
    $passed_url=urldecode(implode('/', $CI->uri->segments));
    ...
    if (isset($post) && $post != '') {
        ...
        if (is_array($url['routed'])&& $passed_url != $url['routed'][$post]){
            redirect($url['routed'][$post]);
        }
    }
    
    if(is_array($url['routed'])&&$url['routed'][get_locale()]!=$passed_url){
        $post=  array_search($passed_url, $url['routed']);
        //Create user lcoale session
        $CI->session->set_userdata('user_locale', $post);
        $locale = $CI->session->userdata('user_locale');
